            ...
            if (data.result.isUploaded) {
                 ...
                 '<div class="close fileinput-remove text-right"></div>' +
                  '<div class="file-preview-thumbnails">' +
                      '<div class="file-preview-frame" id="preview-1409928123859-0">' +
                           '<img  src="/system/Content/Images/Uploads/Products/' + image + '" class="file-preview-image" title="' + image + '" alt="">' +
                      '</div>' +
                  '</div>' +
              ...
              '</div>');
                ...
                $("#imageproduct").on('click', function (event, file, previewId) {
                    $('#imageUpload').hide("slow");
                           '<img  src="/system/Content/Images/Uploads/Products/' + image + '" class="file-preview-image" title="' + image + '" alt="">' +
                    $('#load').html('<img  src="/system/Content/Images/loading.gif"> Please wait....');
                    image = "";
                });
                $('#load').html("");
    ...
    $.ajax({
        ...
        success: function (data) {
            $(data.result).each(function () {
                ...
                $("#price").val(this.price.toFixed(2));
                $("#price").maskMoney('mask');
                $("#weight").val((this.weight != null ? this.weight.toFixed(2) : ""));
                $("#weight").maskMoney('mask');

                $("#id_products").val(id);
                $("#id_brands").val(this.id_brands);
                $("#name").val(this.name);
...
function validateForm() {
    ...
    allFields = $([]).add(name).add(flavour).add(id_brands).add(weight).add(price).add(id_status).add(picture).add(description);

    var bValid = true;
    allFields.removeClass("state-error");

    bValid = bValid && checkLength(name, "Name Field Required", 1, 50);
    bValid = bValid && checkLength(id_brands, "Brands Field Required", 1, 50);
