    ...
    public class BrandsController : WebPageBase
    {
        ...
        public JsonResult CadBrands(td_brands dados)
        {
            try
            {
                ...
                dados.url = this.RemoverAcentos(dados.name).Replace(" ", "-").Replace("  ", "-");
        ...
        public JsonResult UpDateBrands(td_brands dados)
        {
            try
            {
                ...
                result.url = this.RemoverAcentos(dados.name).Replace(" ", "-").Replace("  ", "-");
        ...
        public JsonResult EditBrands(td_brands dados)
        {
            try
            {
                ...
                var result = (from a in db.td_brands
                              where a.id_brands == dados.id_brands && a.id_brands != 1
            ...
            if (myFile != null && myFile.ContentLength != 0)
            {
                ...
                if (this.CreateFolderIfNeeded(pathForSaving))
                {
                    try
                    {
                        ...
                        var name = "Logo_" + DateTime.Now.ToString().Replace("/", "").Replace(":", "").Trim() + "." + array[1];
