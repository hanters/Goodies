        ...
        public virtual ActionResult UploadFile()
        {
            ...
            if (myFile != null && myFile.ContentLength != 0)
            {
                ...
                if (this.CreateFolderIfNeeded(pathForSaving))
                {
                    try
                    {
                        var extencao = myFile.FileName.Split(new char[] { '.' });

                        var name = "Products_" + DateTime.Now.ToString().Replace("/", "").Replace(":", "").Trim() + (extencao.Length > 1 ? "." + extencao[(extencao.Length - 1)] : ".jpg");
