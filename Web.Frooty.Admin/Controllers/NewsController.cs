...
{
    public class NewsController : WebPageBase
    {
        ...
        [HttpPost]
        [ValidateInput(false)]
        public JsonResult CadNewPost(tb_newPost dados)
        {
            try
            {
        ...
        [HttpPost]
        public JsonResult SelectNewPost()
        {
            try
            {
                frootyacai_siteEntities db = new frootyacai_siteEntities();

                var result = (from a in db.tb_newPost
                              ...
                              select new
                              {
                                  ...
                                  a.ts_user_cadm,
                                  c.status
                              }).OrderByDescending(a=> a.id_newPost).ToList();
        ...
        [HttpPost]
        public JsonResult EditNewsPost(tb_newPost dados)
        {
            try
            {
                frootyacai_siteEntities db = new frootyacai_siteEntities();

                var result = (from a in db.tb_newPost where a.id_newPost == dados.id_newPost
        ...
        [HttpPost]
        [ValidateInput(false)]
        public JsonResult upDateNewsPost(tb_newPost dados)
        {
            try
            {
                frootyacai_siteEntities db = new frootyacai_siteEntities();

                var result = db.tb_newPost.Where(a => a.id_newPost == dados.id_newPost).FirstOrDefault();
        ...
        public virtual ActionResult UploadFile()
        {
            ...
            {
                string pathForSaving = Server.MapPath("~/Content/Images/Uploads/News");
                if (this.CreateFolderIfNeeded(pathForSaving))
                {
                    try
                    {
                        var name = "News_" + DateTime.Now.ToString().Replace("/", "").Replace(":", "").Trim() + ".jpg";
