...
$(document).ready(function () {
    ...
    $(".add-cart").on('click', function () {
        ...
        if ($(this).parent().parent().find("input:text").val() == "") {
            ...
            $(this).parent().parent().find("input:text").addClass("state-error");
            return false;
        }

        var dados = {
            id_products: $(this).parent().parent().parent().attr('id-product'),
            amount: $(this).parent().parent().find("input:text").val()
        }

        var obj = this;

        $.ajax({
            url: '/ShopJson/addItens',
            dataType: "json",
            type: "POST",
            data: dados,
            beforeSend: function () {
            ...
            success: function (data) {
                if (data.success) {
                    $(obj).parent().parent().find("input:text").val("");
                    ListCart();
                }
                else {
                    if (data.error == "1") {
        ...
        success: function (data) {
            ...
            if (data.success) {
                ...
                $(data.result).each(function () {
                    ...
                    $("#id_country_").val((this.id_country == 0 ? "" : this.id_country));
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                ...
                $("#hdn-price").val((data.priceShipping != null ? data.priceShipping.toFixed(2) : 0)); // shipping
