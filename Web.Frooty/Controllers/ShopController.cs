    ...
    public class ShopController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var result = (from a in db.td_Products
                          join b in db.td_brands on a.id_brands equals b.id_brands
                          where a.id_status == 2 && a.fl_active == "1" && b.id_brands == 1
                          select a).ToList();
            return View(result);
        }
        ...
        public JsonResult addItens(td_itens_shop dados)
        {
            ...
            {
                //fazer usuario logar
                if (Request.Cookies["acai-user"] == null)
                {
                    return Json(new { success = false, error = 1 }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    if (Request.Cookies["acai-user"].Value == "")
                    {
                        return Json(new { success = false, error = 1 }, JsonRequestBehavior.AllowGet);
                    }
                }

                var partners = Convert.ToInt32(Security.Decrypt(Request.Cookies["acai-user"].Value));

                // verificar se existe pre compra
                if (Request.Cookies["shop-acai"] == null)
                {
                    ...
                    if (shop != null)
                    {
                        ...
                        Response.Cookies.Add(new HttpCookie("shop-acai", Security.Encrypt(shop.id_shop.ToString())));
                    ...
                    else
                    {
                        ...
                        Response.Cookies.Add(new HttpCookie("shop-acai", Security.Encrypt(td_shop.id_shop.ToString())));
                ...
                {
                    var id_shop = Convert.ToInt32(Security.Decrypt(Request.Cookies["shop-acai"].Value));
                    var shop = (from a in db.tb_shop where a.id_shop == id_shop select a).FirstOrDefault();

                    if (shop != null)
                    {
                        dados.id_shop = Convert.ToInt32(Security.Decrypt(Request.Cookies["shop-acai"].Value));
                    }
                    else
                    {
                        var td_shop = new tb_shop
        ...
        public JsonResult ListCart()
        {
            try
            {
                if ((Request.Cookies["shop-acai"] != null) || (Request.Cookies["shop-acai"].Value != ""))
                {
                    var id_shop = Convert.ToInt32(Security.Decrypt(Request.Cookies["shop-acai"].Value));
                    var obj = (from a in db.td_itens_shop
                               join b in db.td_Products on a.id_products equals b.id_products
        ...
        public JsonResult RemoveListCart(int id_iten)
        {
            try
            {
                ...
                var idShop = Convert.ToInt32(Security.Decrypt(Request.Cookies["shop-acai"].Value));
        ...
        public JsonResult Partners()
        {
            try
            {
                var id_partners = (Request.Cookies["acai-user"] == null ? 0 : (Request.Cookies["acai-user"].Value == "" ? 0 : Convert.ToInt32(Security.Decrypt(Request.Cookies["acai-user"].Value))));
        ...
        public JsonResult RulesListCart()
        {
            try
            {
                if ((Request.Cookies["shop-acai"] != null) || (Request.Cookies["shop-acai"].Value != ""))
                {
                    ...
                    var id_shop = Convert.ToInt32(Security.Decrypt(Request.Cookies["shop-acai"].Value));
                    var id_shop_cript = Request.Cookies["shop-acai"].Value;
                    var obj = (from a in db.td_itens_shop
                               join b in db.td_Products on a.id_products equals b.id_products
                               where a.id_shop == id_shop && b.id_status == 2
                               select new
                    ...
                    if (obj.Count > 0)
                    {
                        ...
                        if (OBJresult != null)
                        {
                            ...
                            var partners = (Request.Cookies["acai-user"].Value == null ? 0 : (Request.Cookies["acai-user"].Value == "" ? 0 : Convert.ToInt32(Security.Decrypt(Request.Cookies["acai-user"].Value))));
